...
function StarSelector({
  ...
  return (
    <div
      ...
      data-ocid="review.rating.select"
...
function KitCustomizer({ productId }: { productId: string }) {
  ...
  const toggleItem = (id: string, essential: boolean) => {
    if (essential) return; // cannot toggle essential items
    setSelected((prev) => {
  ...
  return (
    ...
    >
      ...
      <p className="font-body text-sm text-muted-foreground mb-4">
        ...
        <span className="text-saffron font-semibold">Essential</span> are always
        included.
      </p>
            ...
            >
              <label
                ...
                className={`flex items-start gap-3 p-3 rounded-xl border-2 cursor-pointer transition-all duration-200 ${
                  isSelected
                    ? "border-saffron/50 bg-amber-50/60"
                    : "border-border bg-card hover:border-saffron/20"
      ...
      <div className="rounded-xl bg-amber-50 border border-amber-200 p-4">
        <div className="flex items-center justify-between">
          <div>
            <p className="font-body font-semibold text-amber-900 text-sm">
              Kit Summary
            </p>
          ...
          <div className="text-right">
            {selectedCount < totalCount && (
              <p className="font-body text-xs text-amber-700">
                💡 Removing optional items may reduce total
              </p>
            )}
            {selectedCount === totalCount && (
              <p className="font-body text-xs text-green-700 flex items-center gap-1">
                <CheckCircle2 className="w-3.5 h-3.5" /> Full kit selected
              </p>
            )}
          </div>
        </div>
    ...
    >
      <div className="flex items-start justify-between gap-2 mb-2">
        <div>
          ...
          <p className="font-body text-xs text-muted-foreground mt-1 font-mono">
            {truncatedPrincipal}
          </p>
        </div>
        <Badge
          variant="outline"
          className="text-xs border-green-300 text-green-700 bg-green-50 flex items-center gap-1"
        >
      ...
      {review.comment && (
        <p className="font-body text-sm text-foreground leading-relaxed mt-2">
          {review.comment}
        </p>
      )}
    </motion.div>
    ...
    >
      ...
      <div>
        <Label
          ...
          className="font-body text-sm text-muted-foreground mb-2 block"
        >
  ...
  return (
    ...
    >
      <div className="rounded-xl bg-card border border-border hover:border-saffron/30 hover:shadow-card transition-all duration-200 overflow-hidden">
        ...
        <div className="p-3">
          <p className="font-body font-semibold text-sm text-foreground leading-tight line-clamp-2 mb-1">
            {product.name}
          </p>
...
export function ProductDetailPage() {
  ...
  const { data: reviews = [], isLoading: reviewsLoading } =
    useProductReviews(productIdBigInt);
    ...
    <div className="min-h-screen py-8 container mx-auto px-4 max-w-5xl">
      ...
      <Breadcrumb className="mb-6" data-ocid="product.breadcrumb.panel">
        <BreadcrumbList>
          ...
          <BreadcrumbSeparator />
          <BreadcrumbItem>
            <BreadcrumbPage className="font-body text-sm font-semibold">
              {product.name}
            </BreadcrumbPage>
          </BreadcrumbItem>
        </BreadcrumbList>
        ...
        >
          <div className="relative rounded-2xl overflow-hidden bg-gradient-to-br from-amber-50 via-orange-50 to-yellow-50 border border-amber-100 aspect-square flex items-center justify-center shadow-card">
            ...
            <div className="absolute top-3 left-3 flex flex-col gap-1.5">
              ...
              {product.occasionTag && (
                <Badge className="bg-amber-500/90 text-white border-0 text-xs font-body">
                  {product.occasionTag}
                </Badge>
              )}
              {isKit && (
                <Badge className="bg-maroon/90 text-white border-0 text-xs font-body">
                  Customizable Kit
                </Badge>
              )}
          ...
          {product.inStock && (
            <div className="flex items-center gap-3">
              ...
              <div className="flex items-center gap-1 bg-cream-dark rounded-xl p-1">
                ...
                <span className="w-10 text-center font-body font-bold text-foreground text-base">
                  {quantity}
                </span>
                <button
                  type="button"
      ...
      {/* Kit Customizer (for occasion packages only) */}
      {isKit && <KitCustomizer productId={productId} />}
          ...
          <Card className="mb-6 bg-amber-50/50 border-amber-200">
            <CardContent className="p-5 flex items-center gap-6">
              <div className="text-center">
                <div className="font-display text-5xl font-bold text-saffron">
                  {avgRating.toFixed(1)}
                </div>
                ...
                {[5, 4, 3, 2, 1].map((star) => {
                  ...
                  return (
                    <div key={star} className="flex items-center gap-2 mb-1">
                      <span className="font-body text-xs text-muted-foreground w-3">
                        {star}
                      </span>
                      ...
                      <span className="font-body text-xs text-muted-foreground w-4 text-right">
                        {count}
                      </span>
                    </div>
                  );
        ...
        {reviewsLoading ? (
          ...
          <div className="space-y-3 mb-6">
            {reviews.map((review, idx) => (
              <ReviewCard
                key={`${review.reviewer.toString()}-${idx}`}
                review={review}
                index={idx}
              />
            ))}
