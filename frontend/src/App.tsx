...
function LoginStatusBanner() {
  ...
  useEffect(() => {
    if (isLoggingIn) {
      ...
      if (isLoginSuccess) {
        toast.success("Welcome back! You're now logged in.", { duration: 3000 });
      }
      if (isLoginError) {
        toast.error("Login was cancelled or failed. Please try again.", { duration: 4000 });
      }
    }
        ...
        >
          ...
          >
            <span className="inline-flex items-center gap-2">
              ...
              Opening Internet Identity in a new tab — please complete login there
              <span className="w-1.5 h-1.5 rounded-full bg-white/80 animate-pulse" />
            </span>
          </div>
        </motion.div>
...
const myOrdersRoute = createRoute({ getParentRoute: () => rootRoute, path: "/my-orders", component: MyOrdersPage });
const bookPanditRoute = createRoute({ getParentRoute: () => rootRoute, path: "/book-pandit", component: BookPanditPage });
const adminRoute = createRoute({ getParentRoute: () => rootRoute, path: "/admin", component: AdminPage });
const productDetailRoute = createRoute({ getParentRoute: () => rootRoute, path: "/product/$id", component: ProductDetailPage });
const panditDetailRoute = createRoute({ getParentRoute: () => rootRoute, path: "/pandit/$id", component: PanditDetailPage });
