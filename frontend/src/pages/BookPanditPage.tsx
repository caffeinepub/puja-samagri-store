...
function LocationBanner({
  ...
  if (locationState === "granted") {
    return (
      <motion.div
        ...
        className="rounded-xl bg-green-50 border border-green-200 px-4 py-3 flex items-center gap-3"
        data-ocid="location.success_state"
      >
        <CheckCircle2 className="w-5 h-5 text-green-600 flex-shrink-0" />
        <div className="flex-1">
          <p className="font-body text-sm font-semibold text-green-800">
            Showing pandits near {cityName}
          </p>
  ...
  if (locationState === "denied") {
    return (
      <div
        className="rounded-xl bg-muted/50 border border-border px-4 py-3 flex items-center gap-3"
        data-ocid="location.error_state"
      >
        <MapPin className="w-4 h-4 text-muted-foreground flex-shrink-0" />
        <p className="font-body text-sm text-muted-foreground">
          Location access denied. Showing all pandits.
        </p>
      </div>
    );
  }
  ...
  if (locationState === "requesting") {
    return (
      <div className="rounded-xl bg-saffron/10 border border-saffron/20 px-4 py-3 flex items-center gap-3" data-ocid="location.loading_state">
        <Locate className="w-5 h-5 text-saffron animate-pulse flex-shrink-0" />
        <p className="font-body text-sm font-medium text-saffron">Locating you…</p>
      </div>
    );
  }
  ...
  return (
    <motion.div
      initial={{ opacity: 0, y: -8 }}
      animate={{ opacity: 1, y: 0 }}
      className="rounded-xl overflow-hidden border border-saffron/30"
      style={{
        background:
          "linear-gradient(135deg, oklch(0.72 0.18 60 / 0.15) 0%, oklch(0.68 0.22 45 / 0.1) 100%)",
      }}
      ...
      <div className="px-5 py-4 flex items-center gap-4 flex-wrap">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="font-body text-xs text-muted-foreground">
            Allow location access to see nearby pandits sorted by distance
          </p>
        </div>
        <Button
          size="sm"
          onClick={onAllow}
          className="bg-saffron hover:bg-saffron-dark text-white font-body font-semibold shrink-0"
    ...
    >
      <Card className="h-full flex flex-col bg-card border-border hover:shadow-card transition-all duration-300 hover:-translate-y-0.5 overflow-hidden">
        ...
        <CardContent className="p-5 flex-1 flex flex-col gap-4">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-1.5 mt-1 flex-wrap">
                <div className="flex items-center gap-1">
                  ...
                  <span className="font-body text-xs text-muted-foreground">{pandit.area}</span>
                </div>
                {distance !== undefined && (
                  <Badge className="text-xs bg-blue-50 text-blue-700 border-blue-200 flex items-center gap-0.5 px-1.5">
              ...
              <div className="flex items-center gap-2 mt-1.5 flex-wrap">
                ...
                <span className="font-body text-xs font-semibold text-foreground">{pandit.rating.toFixed(1)}</span>
                <span className="font-body text-xs text-muted-foreground">({pandit.reviewCount} reviews)</span>
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center justify-between mt-auto pt-2 border-t border-border gap-2 flex-wrap">
            <div>
              <span className="font-display text-lg font-bold text-saffron">
                ₹{pandit.pricePerPooja.toLocaleString()}
              </span>
              <span className="font-body text-xs text-muted-foreground ml-1">/pooja</span>
            </div>
            <div className="flex items-center gap-2">
              <Button
                size="sm"
                variant="outline"
                className="border-saffron/40 text-saffron hover:bg-saffron/5 font-body font-semibold text-xs"
                onClick={() => navigate({ to: "/pandit/$id", params: { id: pandit.id } })}
                data-ocid={`pandits.view.button.${index + 1}`}
              >
...
function BookingForm({
  ...
  function validate(): boolean {
    ...
    if (!form.ceremonyType) newErrors.ceremonyType = "Please select a ceremony type";
    ...
    if (!form.contactName.trim()) newErrors.contactName = "Please enter your name";
      ...
      <Card className="border-saffron/30 bg-gradient-to-br from-amber-50/50 to-orange-50/30">
        <CardHeader className="pb-4">
          <div className="flex items-center justify-between">
            <CardTitle className="font-display text-xl text-foreground">Book a Pooja</CardTitle>
            <Button
              variant="ghost"
              size="sm"
              onClick={onCancel}
          ...
          <div className="flex items-center gap-3 bg-saffron/10 rounded-lg px-4 py-3 border border-saffron/20">
            ...
            <div>
              <p className="font-display text-sm font-bold text-foreground">{pandit.name}</p>
              <p className="font-body text-xs text-muted-foreground">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-1.5">
                ...
                <Select value={form.ceremonyType} onValueChange={(val) => setForm((p) => ({ ...p, ceremonyType: val }))}>
                  <SelectTrigger className="font-body" data-ocid="booking.ceremonytype.select">
                    <SelectValue placeholder="Select ceremony" />
                  </SelectTrigger>
                  <SelectContent>
                    {CEREMONY_TYPES.map((ct) => (
                      <SelectItem key={ct} value={ct} className="font-body">{ct}</SelectItem>
                    ))}
                  </SelectContent>
                </Select>
                {errors.ceremonyType && (
                  <p className="font-body text-xs text-destructive" data-ocid="booking.ceremonytype.error_state">
                    {errors.ceremonyType}
                  </p>
                )}
              </div>
              ...
              <div className="space-y-1.5">
                ...
                />
                {errors.date && (
                  <p className="font-body text-xs text-destructive" data-ocid="booking.date.error_state">{errors.date}</p>
                )}
              </div>
              ...
              <div className="space-y-1.5">
                ...
                <Select value={form.timeSlot} onValueChange={(val) => setForm((p) => ({ ...p, timeSlot: val }))}>
                  <SelectTrigger className="font-body" data-ocid="booking.timeslot.select">
                ...
                {errors.timeSlot && (
                  <p className="font-body text-xs text-destructive" data-ocid="booking.timeslot.error_state">{errors.timeSlot}</p>
                )}
              </div>
              ...
              <div className="space-y-1.5">
                ...
                />
                {errors.contactName && (
                  <p className="font-body text-xs text-destructive" data-ocid="booking.contactname.error_state">{errors.contactName}</p>
                )}
              </div>
              ...
              <div className="space-y-1.5">
                ...
                />
                {errors.phone && (
                  <p className="font-body text-xs text-destructive" data-ocid="booking.phone.error_state">{errors.phone}</p>
                )}
              </div>
            </div>
            ...
            <div className="space-y-1.5">
              ...
              />
              {errors.address && (
                <p className="font-body text-xs text-destructive" data-ocid="booking.address.error_state">{errors.address}</p>
              )}
            </div>
      ...
      >
        <CardContent className="p-8 text-center">
          ...
          <p className="font-body text-sm text-muted-foreground mb-6">
            Your pooja has been successfully scheduled. The pandit will contact you shortly.
          </p>
          <div className="bg-white/80 rounded-xl p-4 text-left space-y-3 mb-6 border border-green-200">
            <div className="flex items-center gap-3">
              <span className="text-2xl">{pandit.emoji}</span>
              <div>
                <p className="font-display text-sm font-bold text-foreground">{booking.panditName}</p>
                <p className="font-body text-xs text-muted-foreground">{pandit.area}</p>
              </div>
            </div>
            <div className="grid grid-cols-2 gap-3 text-sm font-body">
              <div className="flex items-center gap-2 text-muted-foreground">
                <CalendarDays className="w-4 h-4 text-saffron" />
                <span>{new Date(booking.date).toLocaleDateString("en-IN", { dateStyle: "long" })}</span>
              </div>
              <div className="flex items-center gap-2 text-muted-foreground">
            ...
            <div className="pt-2 border-t border-green-100">
              <p className="font-body text-xs font-semibold text-saffron">Ceremony: {booking.ceremonyType}</p>
              {booking.notes && (
                <p className="font-body text-xs text-muted-foreground mt-1">Notes: {booking.notes}</p>
              )}
            </div>
          </div>
          <Button
          ...
          <div className="flex items-start justify-between gap-3 flex-wrap">
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 flex-wrap mb-2">
                <h4 className="font-display text-sm font-bold text-foreground">{booking.panditName}</h4>
                <Badge className={`${config.class} text-xs`}>{config.label}</Badge>
              </div>
              <p className="font-body text-xs font-semibold text-saffron mb-2">{booking.ceremonyType}</p>
            ...
            {booking.status === "confirmed" && (
              <AlertDialog>
                ...
                <AlertDialogContent data-ocid="mybookings.cancel.dialog">
                  <AlertDialogHeader>
                    <AlertDialogTitle className="font-display">Cancel Booking?</AlertDialogTitle>
                    <AlertDialogDescription className="font-body">
                      Are you sure you want to cancel the booking with{" "}
                      <strong>{booking.panditName}</strong> for <strong>{booking.ceremonyType}</strong> on{" "}
                      <strong>{new Date(booking.date).toLocaleDateString("en-IN", { dateStyle: "medium" })}</strong>?
                      This action cannot be undone.
                    </AlertDialogDescription>
                  </AlertDialogHeader>
                  <AlertDialogFooter>
                    <AlertDialogCancel className="font-body" data-ocid="mybookings.cancel.cancel_button">
                      Keep Booking
                    </AlertDialogCancel>
                    <AlertDialogAction
...
export function BookPanditPage() {
  ...
  const [confirmedBooking, setConfirmedBooking] = useState<{ booking: PanditBooking; pandit: Pandit } | null>(null);
  ...
  function handleAllowLocation() {
    ...
    navigator.geolocation.getCurrentPosition(
      (pos) => {
        ...
        // Approximate city name based on coordinates (Mumbai area)
        if (latitude > 19.1 && latitude < 19.3 && longitude > 72.8 && longitude < 73.0) {
          setCityName("Mumbai");
        } else if (latitude > 19.0 && latitude < 19.4) {
          setCityName("Mumbai region");
        } else {
  ...
  // Compute distances and sort pandits if location is granted
  const panditsWithDistance = panditsWithAvailability.map((p) => ({
    pandit: p,
    distance:
      userCoords !== null
        ? getDistanceKm(userCoords.lat, userCoords.lng, p.lat, p.lng)
        : undefined,
  }));

  const sortedPandits =
    locationState === "granted"
      ? [...panditsWithDistance].sort((a, b) => (a.distance ?? 999) - (b.distance ?? 999))
      : panditsWithDistance;
  ...
  function handleConfirmBooking(data: BookingFormData) {
    ...
    <div className="min-h-screen">
        ...
        <div className="container mx-auto relative">
          ...
          >
            ...
            <h1 className="font-display text-4xl sm:text-5xl font-bold text-white mb-4 leading-tight">
              Book a Pandit for Your <span className="text-amber-300">Puja</span>
            </h1>
            <p className="font-body text-base text-white/80 leading-relaxed">
              Connect with experienced Vedic pandits in your area for Satyanarayan Katha, Griha Pravesh, weddings, havans & more.
            </p>
          </motion.div>
        </div>
      </section>
      ...
      <div className="container mx-auto px-4 py-10">
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          <TabsList className="mb-8 bg-cream-dark p-1 rounded-xl" data-ocid="bookpandit.tabs.tab">
            <TabsTrigger
              value="find"
            ...
            <div className="space-y-6">
              ...
              {!confirmedBooking && (
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
                  {sortedPandits.map(({ pandit, distance }, idx) => (
                    <PanditCard
                      key={pandit.id}
                      pandit={pandit}
                      index={idx}
              ...
              {selectedPandit && !confirmedBooking && identity && (
                <div ref={formRef}>
                  <h2 className="font-display text-2xl font-bold text-foreground mb-4">Complete Your Booking</h2>
                  <BookingForm
            ...
            {bookings.length === 0 ? (
              ...
              <div className="space-y-4 max-w-2xl">
                ...
                {[...bookings].reverse().map((booking, idx) => (
                  <BookingListItem key={booking.id} booking={booking} index={idx} onCancel={cancelBooking} />
                ))}
              </div>
            )}
          </TabsContent>
